...
public class PlantList {
        ...
        try (Scanner scanner = new Scanner(new BufferedReader(new FileReader(pathAndFile)))) {
            ...
            while (scanner.hasNextLine()) {
                ...
                try {
                    ...
                    throw new PlantException("Neplatný vstup soubor " + pathAndFile + " na řádek " + lineNumber + ":\n\t" + e.getLocalizedMessage());
                }
            }
    ...
    public void exportToFile(String pathAndFile) throws PlantException {
        int lineNumber = 0;
        try (PrintWriter write = new PrintWriter(new BufferedWriter(new FileWriter(pathAndFile)))) {
            for (Plant plant : plants) {
                String plantAsFileLine = plant.prepareOutputString(FILE_ITEM_DELIMITER);
                write.println(plantAsFileLine);
                lineNumber++;
            }
        } catch (IOException e) {
